...
interface IAgent {
    ...
    function addMiner(SignedCredential calldata sc) external;

    function removeMiner(uint64 newMinerOwner, SignedCredential calldata sc) external;

    function migrateMiner(uint64 miner) external;

    function changeMinerWorker(uint64 miner, uint64 worker, uint64[] calldata controlAddresses) external;
    ...
    function withdraw(address receiver, SignedCredential calldata signedCred) external;

    function borrow(uint256 poolID, SignedCredential calldata signedCred) external;

    function pay(uint256 poolID, SignedCredential calldata signedCred)
        external
        returns (uint256 rate, uint256 epochsPaid, uint256 principalPaid, uint256 refund);

    function pullFunds(SignedCredential calldata signedCred) external;

    function pushFunds(SignedCredential calldata signedCred) external;
}
