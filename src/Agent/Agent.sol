...
contract Agent is IAgent, Operatable {
    ...
    constructor(uint256 agentID, address agentRouter, address owner, address operator, address apiRequestKey)
     ...
     */
    function addMiner(SignedCredential calldata sc) external onlyOwnerOperator validateAndBurnCred(sc) checkVersion {
     ...
     */
    function removeMiner(uint64 newMinerOwner, SignedCredential calldata sc)
        external
        onlyOwner
        notPaused
        notOnAdministration
     ...
     */
    function prepareMinerForLiquidation(uint64 miner, uint64 liquidator) external onlyAgentPolice {
        if (!defaulted) revert Unauthorized();
        miner.changeOwnerID(liquidator);
    }
     ...
     */
    function changeMinerWorker(uint64 miner, uint64 worker, uint64[] calldata controlAddresses) external checkVersion {
        if (msg.sender != owner && msg.sender != administration) revert Unauthorized();
     ...
     * @dev miner must be owned by this Agent in order for this call to execute
     */
    function confirmChangeMinerWorker(uint64 miner) external checkVersion {
        if (msg.sender != owner && msg.sender != administration) revert Unauthorized();
     ...
     */
    function withdraw(address receiver, SignedCredential calldata sc)
        external
        onlyOwner
        notPaused
        notInDefault
     ...
     */
    function pullFunds(SignedCredential calldata sc) external onlyOwnerOperator validateAndBurnCred(sc) checkVersion {
     ...
     */
    function borrow(uint256 poolID, SignedCredential calldata sc)
        external
        onlyOwner
        notPaused
        notInDefault
     ...
     */
    function pay(uint256 poolID, SignedCredential calldata sc)
        external
        onlyOwnerOperator
        validateAndBurnCred(sc)
        checkVersion
    ...
    function _validateAndBurnCred(SignedCredential calldata signedCredential) internal {
    ...
    function _onlyOwnerOperatorOverride() internal view {
        ...
        if (msg.sender != owner && msg.sender != operator && msg.sender != administration) revert Unauthorized();
    }
