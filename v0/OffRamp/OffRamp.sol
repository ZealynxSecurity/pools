...
contract SimpleRamp is IOffRamp {
    ...
    modifier poolNotUpgraded() {
        if (pool.isShuttingDown()) {
            if (address(pool) != address(GetRoute.pool(GetRoute.poolRegistry(router), poolID))) revert Unauthorized();
        }
        _;
    }
    ...
    function maxWithdraw(address account) external view poolNotUpgraded returns (uint256) {
        return Math.min(pool.convertToAssets(iFIL.balanceOf(account)), pool.getLiquidAssets());
    }
    ...
    function previewWithdraw(uint256 assets) public view poolNotUpgraded returns (uint256 shares) {
        if (assets > pool.getLiquidAssets()) return 0;

        uint256 supply = iFIL.totalSupply(); // Saves an extra SLOAD if totalSupply is non-zero.

        shares = supply == 0 ? assets : assets.mulDivUp(supply, pool.totalAssets());
    }

    function maxRedeem(address account) external view poolNotUpgraded returns (uint256 shares) {
    ...
    function previewRedeem(uint256 shares) public view poolNotUpgraded returns (uint256 assets) {
        uint256 supply = iFIL.totalSupply(); // Saves an extra SLOAD if totalSupply is non-zero.

        assets = supply == 0 ? shares : shares.mulDivUp(pool.totalAssets(), supply);
