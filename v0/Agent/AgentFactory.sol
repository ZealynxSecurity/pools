...
import {ROUTE_VC_ISSUER, ROUTE_MINER_REGISTRY} from "v0/Constants/Routes.sol";
...
contract AgentFactory is IAgentFactory {
    ...
    function create(address owner, address operator, address adoRequestKey) external returns (address agent) {
        agentCount++;
        agent = GetRoute.agentDeployer(router).deploy(router, agentCount, owner, operator, adoRequestKey);
    ...
    function upgradeAgent(address agent) external returns (address newAgent) {
        ...
        newAgent =
            agDeployer.deploy(router, agentId, owner, IAuth(address(oldAgent)).operator(), oldAgent.adoRequestKey());
