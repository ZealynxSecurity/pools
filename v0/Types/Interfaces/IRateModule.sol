...
interface IRateModule {
    ...
    function computeLTV(uint256 totalPrincipal, uint256 collateralValue) external pure returns (uint256 ltv);
    ...
    function computeDTE(uint256 accountPrincipal, uint256 agentValue) external pure returns (uint256 dte);
