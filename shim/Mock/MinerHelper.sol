...
library MinerHelper {
    ...
    function changeOwnerID(uint64 target, uint64 newMinerOwnerID) internal {
        IMockMiner(MockIDAddrStore(ID_STORE_ADDR).ids(target)).changeOwnerAddress(
            MockIDAddrStore(ID_STORE_ADDR).ids(newMinerOwnerID)
        );
    }
     ...
     */
    function changeWorkerAddress(uint64 target, uint64 workerAddr, uint64[] memory controllerAddrs) internal {}
